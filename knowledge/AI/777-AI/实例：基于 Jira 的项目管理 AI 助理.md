---
title: "实例：基于 Jira 的项目管理 AI 助理"
source: "实例：基于 Jira 的项目管理 AI 助理.docx"
type: docx
tags: ["docx", "ai"]
path: "777-AI-技术"
created: 2026-07-03
...
![[实例：基于 Jira 的项目管理 AI 助理-b84e3bbb.docx]]
...
你的远程 Jira MCP Server 运行在一个云端服务器上，它不会直接暴露你所有的 Jira 数据，而是暴露以下能力：
  ...
  **Tool**       search_jira_issues(project_key, status)   允许 AI **执行操作**：查询特定项目的任务。
...
整个过程涉及到 MCP 客户端、LLM 和远程 MCP Server 之间的交互：
  ...
  **Step 2: 意图推理与工具调用**     **AI Agent (MCP Client + LLM)**   LLM 推理得知：回答这个问题需要访问 Jira 系统。它决定调用 search_jira_issues 这个 **Tool**，并构造参数：project_key=\'我的项目\', status=\'待办\', type=\'Bug\'。                                                                                     LLM 知道调用 Tool 后会返回 **Resources** 作为上下文。
